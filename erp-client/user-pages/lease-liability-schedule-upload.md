...
2. Use the type-ahead control to select the lease liability that the CSV belongs to. The server now creates the amortisation schedule and compilation context using that single selection, so no additional dropdowns are required.
