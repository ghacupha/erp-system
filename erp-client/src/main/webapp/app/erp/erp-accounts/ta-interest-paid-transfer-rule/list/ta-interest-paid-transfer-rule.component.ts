...
@Component({
  ...
  templateUrl: './ta-interest-paid-transfer-rule.component.html',
})
export class TAInterestPaidTransferRuleComponent implements OnInit {
  tAInterestPaidTransferRules?: ITAInterestPaidTransferRule[];
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.tAInterestPaidTransferRuleService
        .search({
          page: pageToLoad - 1,
        ...
        .subscribe(
          (res: HttpResponse<ITAInterestPaidTransferRule[]>) => {
  ...
  trackId(index: number, item: ITAInterestPaidTransferRule): number {
    return item.id!;
  }

  delete(tAInterestPaidTransferRule: ITAInterestPaidTransferRule): void {
    const modalRef = this.modalService.open(TAInterestPaidTransferRuleDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.tAInterestPaidTransferRule = tAInterestPaidTransferRule;
  ...
  protected onSuccess(data: ITAInterestPaidTransferRule[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/ta-interest-paid-transfer-rule'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.tAInterestPaidTransferRules = data ?? [];
    this.ngbPaginationPage = this.page;
  }
