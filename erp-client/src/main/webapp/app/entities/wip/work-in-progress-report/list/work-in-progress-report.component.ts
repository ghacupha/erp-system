...
export class WorkInProgressReportComponent implements OnInit {
  ...
  protected onSuccess(data: IWorkInProgressReport[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/work-in-progress-report'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.workInProgressReports = data ?? [];
    this.ngbPaginationPage = this.page;
  }
