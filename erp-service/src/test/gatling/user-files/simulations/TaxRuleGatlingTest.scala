...
/**
 * Performance test for the TaxRule entity.
 */
class TaxRuleGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the TaxRule entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all taxRules")
            .get("/services/erpservice/api/tax-rules")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new taxRule")
            .post("/services/erpservice/api/tax-rules")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "paymentNumber":"SAMPLE_TEXT"
                , "paymentDate":"2020-01-01T00:00:00.000Z"
                , "telcoExciseDuty":null
                , "valueAddedTax":null
                , "withholdingVAT":null
                , "withholdingTaxConsultancy":null
                , "withholdingTaxRent":null
                , "cateringLevy":null
                , "serviceCharge":null
                , "withholdingTaxImportedService":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_taxRule_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created taxRule")
                .get("/services/erpservice${new_taxRule_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created taxRule")
            .delete("/services/erpservice${new_taxRule_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
