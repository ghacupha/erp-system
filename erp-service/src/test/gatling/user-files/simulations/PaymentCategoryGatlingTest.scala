...
/**
 * Performance test for the PaymentCategory entity.
 */
class PaymentCategoryGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PaymentCategory entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all paymentCategories")
            .get("/services/erpservice/api/payment-categories")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new paymentCategory")
            .post("/services/erpservice/api/payment-categories")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "categoryName":"SAMPLE_TEXT"
                , "categoryDescription":"SAMPLE_TEXT"
                , "categoryType":"CATEGORY0"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_paymentCategory_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created paymentCategory")
                .get("/services/erpservice${new_paymentCategory_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created paymentCategory")
            .delete("/services/erpservice${new_paymentCategory_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
