...
/**
 * Mapper for the entity {@link WIPListReport} and its DTO {@link WIPListReportDTO}.
 */
@Mapper(componentModel = "spring", uses = { ApplicationUserMapper.class })
public interface WIPListReportMapper extends EntityMapper<WIPListReportDTO, WIPListReport> {
    @Mapping(target = "requestedBy", source = "requestedBy", qualifiedByName = "applicationIdentity")
    WIPListReportDTO toDto(WIPListReport s);
}
