 ...
 */
import io.github.erp.erp.leases.reconciliation.LeaseSettlementPostingContextDTO;
import io.github.erp.erp.leases.reconciliation.LeaseSettlementReconciliationServiceExtension;
import io.github.erp.internal.repository.InternalLeaseSettlementRepository;
import io.github.erp.internal.service.leases.InternalLeaseSettlementService;
...
public class LeaseSettlementResourceProd {

    private final Logger log = LoggerFactory.getLogger(LeaseSettlementResourceProd.class);
    ...
    private final InternalLeaseSettlementService leaseSettlementService;

    private final InternalLeaseSettlementRepository leaseSettlementRepository;

    private final LeaseSettlementQueryService leaseSettlementQueryService;

    private final LeaseSettlementReconciliationServiceExtension reconciliationService;

    public LeaseSettlementResourceProd(
        InternalLeaseSettlementService leaseSettlementService,
        InternalLeaseSettlementRepository leaseSettlementRepository,
        LeaseSettlementQueryService leaseSettlementQueryService,
        LeaseSettlementReconciliationServiceExtension reconciliationService
    ) {
    ...
    /**
     * {@code GET  /lease-settlements/reconcile/:periodId} : reconcile lease settlements for a period.
     *
     * @param periodId the period id.
     * @param postedById the application user id.
     * @return the posting contexts for reconciliation.
     */
    @GetMapping("/lease-settlements/reconcile/{periodId}")
    public ResponseEntity<List<LeaseSettlementPostingContextDTO>> reconcileLeaseSettlements(
        @PathVariable Long periodId,
        @RequestParam(required = false) Long postedById
    ) {
        log.debug("REST request to reconcile LeaseSettlements for period {}", periodId);
        List<PostingContext> contexts = reconciliationService.reconcilePeriod(periodId, postedById);
        List<LeaseSettlementPostingContextDTO> response = contexts.stream().map(this::toDto).collect(Collectors.toList());
        return ResponseEntity.ok(response);
    }

    private LeaseSettlementPostingContextDTO toDto(PostingContext context) {
        LeaseSettlementPostingContextDTO dto = new LeaseSettlementPostingContextDTO();
        dto.setModule(context.getModule());
        dto.setEventType(context.getEventType());
        dto.setVarianceType(context.getVarianceType());
        dto.setInvoiceTiming(context.getInvoiceTiming());
        dto.setAttributes(context.getAttributes());
        dto.setAmount(context.getAmount());
        dto.setTransactionDate(context.getTransactionDate());
        dto.setDescription(context.getDescription());
        dto.setTransactionType(context.getTransactionType());
        dto.setPostingId(context.getPostingId());
        return dto;
    }
}
