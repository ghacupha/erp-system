...
/**
 * Search-only, flattened projection of {@link AssetRegistration} and everything it relates to
 * (asset category, dealer, accessories, invoices, purchase orders, business documents,
 * warranties, acquiring transaction). This document exists purely so a search can match on any
 * of those related entities' fields without Elasticsearch ever needing to hold - or fail to
 * hold - the real, deeply-relational {@link AssetRegistration} graph itself.
 *
 * Deliberately NOT date-partitioned like {@code AssetRegistration}'s own index
 * ("assetregistration-yyyy-MM") - a fixed index name means there is nothing to roll over and
 * nothing that can go missing on the first of a new month.
 *
 * Search results from this document are never returned to a client directly - callers take the
 * matched ids and re-fetch the real {@link AssetRegistration} rows from Postgres (see
 * InternalAssetRegistrationServiceImpl#search), so a stale or wrongly-indexed document here can
 * at worst cause a false-positive match, never a false result.
 */
@Document(indexName = "assetregistrationindex")
public class AssetRegistrationIndex implements Serializable {
    ...
    @Field(type = FieldType.Keyword)
    private String acquiringTransactionPaymentNumber;

    @Field(type = FieldType.Text)
    private List<String> designatedUserNames;

    @Field(type = FieldType.Text)
    private List<String> accessoryDescriptions;
    ...
    public String getAcquiringTransactionPaymentNumber() {
        return acquiringTransactionPaymentNumber;
    }

    public void setAcquiringTransactionPaymentNumber(String acquiringTransactionPaymentNumber) {
        this.acquiringTransactionPaymentNumber = acquiringTransactionPaymentNumber;
    }
    ...
    public List<String> getAccessoryDescriptions() {
        return accessoryDescriptions;
    }

    public void setAccessoryDescriptions(List<String> accessoryDescriptions) {
