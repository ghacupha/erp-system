...
/**
 * REST controller for managing {@link WIPListItem}.
 */
@RestController
@RequestMapping("/api")
public class WIPListItemResource {

    private final Logger log = LoggerFactory.getLogger(WIPListItemResource.class);

    private final InternalWIPListItemService wIPListItemService;

    public WIPListItemResource(
    ...
    public ResponseEntity<List<WIPListItemDTO>> getAllWIPListItems(WIPListItemCriteria criteria, Pageable pageable) {
        ...
        Page<WIPListItemDTO> page = wIPListItemService.findByCriteria(criteria, pageable);
