...
class AssetWriteOffResourceIT {
    ...
    @Test
    @Transactional
    void getAllAssetWriteOffsByAccessedByIsEqualToSomething() throws Exception {
        // Initialize the database
        assetWriteOffRepository.saveAndFlush(assetWriteOff);
        ApplicationUser accessedBy;
        if (TestUtil.findAll(em, ApplicationUser.class).isEmpty()) {
            accessedBy = ApplicationUserResourceIT.createEntity(em);
            em.persist(accessedBy);
            em.flush();
        } else {
            accessedBy = TestUtil.findAll(em, ApplicationUser.class).get(0);
        }
        em.persist(accessedBy);
        em.flush();
        assetWriteOff.setAccessedBy(accessedBy);
        assetWriteOffRepository.saveAndFlush(assetWriteOff);
        Long accessedById = accessedBy.getId();

        // Get all the assetWriteOffList where accessedBy equals to accessedById
        defaultAssetWriteOffShouldBeFound("accessedById.equals=" + accessedById);

        // Get all the assetWriteOffList where accessedBy equals to (accessedById + 1)
        defaultAssetWriteOffShouldNotBeFound("accessedById.equals=" + (accessedById + 1));
    }
    ...
    void getAllAssetWriteOffsByPlaceholderIsEqualToSomething() throws Exception {
        ...
        em.persist(placeholder);
        em.flush();
        assetWriteOff.setPlaceholder(placeholder);
        assetWriteOffRepository.saveAndFlush(assetWriteOff);
