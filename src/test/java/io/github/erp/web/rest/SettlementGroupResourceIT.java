...
/**
 * Integration tests for the {@link SettlementGroupResource} REST controller.
 */
@IntegrationTest
...
public class SettlementGroupResourceIT {
    ...
    @Test
    @Transactional
    void createSettlementGroupWithExistingId() throws Exception {
        settlementGroup.setId(1L);
        SettlementGroupDTO settlementGroupDTO = settlementGroupMapper.toDto(settlementGroup);
        ...
        List<SettlementGroup> settlementGroupList = settlementGroupRepository.findAll();
        assertThat(settlementGroupList).hasSize(databaseSizeBeforeCreate);

        verify(mockSettlementGroupSearchRepository, times(0)).save(settlementGroup);
    }

    @Test
    @Transactional
    void checkGroupNameIsRequired() throws Exception {
        int databaseSizeBeforeTest = settlementGroupRepository.findAll().size();
        // set the field null
        settlementGroup.setGroupName(null);
        ...
        List<SettlementGroup> settlementGroupList = settlementGroupRepository.findAll();
        assertThat(settlementGroupList).hasSize(databaseSizeBeforeTest);
    }
    ...
    @Test
    @Transactional
    void getNonExistingSettlementGroup() throws Exception {
        restSettlementGroupMockMvc.perform(get(ENTITY_API_URL_ID, Long.MAX_VALUE)).andExpect(status().isNotFound());
    }

    @Test
    @Transactional
    void putNewSettlementGroup() throws Exception {
        // Initialize the database
        settlementGroupRepository.saveAndFlush(settlementGroup);

        int databaseSizeBeforeUpdate = settlementGroupRepository.findAll().size();

        SettlementGroup updatedSettlementGroup = settlementGroupRepository.findById(settlementGroup.getId()).get();
        // Disconnect from session so that the updates on updatedSettlementGroup are not directly saved in db
        em.detach(updatedSettlementGroup);
        updatedSettlementGroup.groupName(UPDATED_GROUP_NAME).description(UPDATED_DESCRIPTION).remarks(UPDATED_REMARKS);
        SettlementGroupDTO settlementGroupDTO = settlementGroupMapper.toDto(updatedSettlementGroup);

        restSettlementGroupMockMvc
            .perform(
                put(ENTITY_API_URL_ID, settlementGroupDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(settlementGroupDTO))
            )
            .andExpect(status().isOk());

        List<SettlementGroup> settlementGroupList = settlementGroupRepository.findAll();
        assertThat(settlementGroupList).hasSize(databaseSizeBeforeUpdate);
        SettlementGroup testSettlementGroup = settlementGroupList.get(settlementGroupList.size() - 1);
        assertThat(testSettlementGroup.getGroupName()).isEqualTo(UPDATED_GROUP_NAME);
        assertThat(testSettlementGroup.getDescription()).isEqualTo(UPDATED_DESCRIPTION);
        assertThat(testSettlementGroup.getRemarks()).isEqualTo(UPDATED_REMARKS);

        verify(mockSettlementGroupSearchRepository).save(testSettlementGroup);
    }

    @Test
    @Transactional
    void deleteSettlementGroup() throws Exception {
        // Initialize the database
        settlementGroupRepository.saveAndFlush(settlementGroup);

        int databaseSizeBeforeDelete = settlementGroupRepository.findAll().size();

        restSettlementGroupMockMvc
            .perform(delete(ENTITY_API_URL_ID, settlementGroup.getId()).accept(MediaType.APPLICATION_JSON))
            .andExpect(status().isNoContent());

        // Validate the database contains one less item
        List<SettlementGroup> settlementGroupList = settlementGroupRepository.findAll();
        assertThat(settlementGroupList).hasSize(databaseSizeBeforeDelete - 1);

        verify(mockSettlementGroupSearchRepository, times(1)).deleteById(settlementGroup.getId());
    }
