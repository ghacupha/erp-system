...
export class TaxRuleComponentsPage {
  ...
  deleteButtons = element.all(by.css('gha-tax-rule div table .btn-danger'));
  title = element.all(by.css('gha-tax-rule div h2#page-heading span')).first();
...
export class TaxRuleUpdatePage {
  pageTitle = element(by.id('gha-tax-rule-heading'));
...
export class TaxRuleDeleteDialog {
  private dialogTitle = element(by.id('gha-delete-taxRule-heading'));
  private confirmButton = element(by.id('gha-confirm-delete-taxRule'));
