...
export class PaymentComponentsPage {
  ...
  deleteButtons = element.all(by.css('gha-payment div table .btn-danger'));
  title = element.all(by.css('gha-payment div h2#page-heading span')).first();
...
export class PaymentUpdatePage {
  pageTitle = element(by.id('gha-payment-heading'));
...
export class PaymentDeleteDialog {
  private dialogTitle = element(by.id('gha-delete-payment-heading'));
  private confirmButton = element(by.id('gha-confirm-delete-payment'));
