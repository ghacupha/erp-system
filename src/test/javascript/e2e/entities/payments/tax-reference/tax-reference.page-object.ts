...
export class TaxReferenceComponentsPage {
  ...
  deleteButtons = element.all(by.css('gha-tax-reference div table .btn-danger'));
  title = element.all(by.css('gha-tax-reference div h2#page-heading span')).first();
...
export class TaxReferenceUpdatePage {
  pageTitle = element(by.id('gha-tax-reference-heading'));
...
export class TaxReferenceDeleteDialog {
  private dialogTitle = element(by.id('gha-delete-taxReference-heading'));
  private confirmButton = element(by.id('gha-confirm-delete-taxReference'));
