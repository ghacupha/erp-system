...
@Service
@Transactional
public class InternalFiscalMonthServiceImpl implements InternalFiscalMonthService {

    private final Logger log = LoggerFactory.getLogger(InternalFiscalMonthServiceImpl.class);
    ...
    public InternalFiscalMonthServiceImpl(
     ...
     */
    @Override
    public Optional<FiscalMonthDTO> findOneAfterXPeriods(long currentFiscalMonthId, int fiscalPeriods) {
        log.debug("Request for period, after {} periods have lapsed starting periodId: {}", fiscalPeriods, currentFiscalMonthId);
        return fiscalMonthRepository.getNextFiscalPeriodAfterLapsedMonths(currentFiscalMonthId, fiscalPeriods).map(fiscalMonthMapper::toDto);
    }
}
