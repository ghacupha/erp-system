...
//    Page<WorkInProgressOutstandingReport> findByReportDate(@Param("reportDate") String reportDate, Pageable pageable);
    ...
    List<WorkInProgressOutstandingReportREPO> findByReportDate(@Param("reportDate") LocalDate reportDate, Pageable pageable);

//    @Query("SELECT NEW io.github.erp.service.dto.WorkInProgressOutstandingReportDTO(" +
//        "w.id, " +
//        "w.sequenceNumber, " +
//        "w.particulars, " +
//        "d.dealerName, " +
//        "c.iso4217CurrencyCode, " +
//        "w.instalmentAmount," +
//        "COALESCE(SUM(ta.transferAmount), 0)," +
//        "(w.instalmentAmount - COALESCE(SUM(ta.transferAmount), 0))) " +
//        "FROM WorkInProgressRegistration w " +
//        "JOIN Dealer d ON d.id = w.dealer.id " +
//        "JOIN SettlementCurrency c ON c.id = w.settlementCurrency.id " +
//        "JOIN Settlement s ON s.id = w.settlementTransaction.id " +
...
//        "AND (ta.transferDate IS NULL OR ta.transferDate <= :reportDate) " +
//        "GROUP BY w.id, w.sequenceNumber, w.particulars, d.dealerName, c.iso4217CurrencyCode, w.instalmentAmount, ta.transferAmount, ta.transferAmount")
//    List<WorkInProgressOutstandingReportDTO> findByReportDate(@Param("reportDate") LocalDate reportDate, Pageable pageable);
}
