 ...
 */
@Mapper(componentModel = "spring", uses = { InstitutionCodeMapper.class, IsoCurrencyCodeMapper.class })
public interface ExchangeRateMapper extends EntityMapper<ExchangeRateDTO, ExchangeRate> {
    @Mapping(target = "institutionCode", source = "institutionCode", qualifiedByName = "institutionName")
    @Mapping(target = "currencyCode", source = "currencyCode", qualifiedByName = "alphabeticCode")
    ExchangeRateDTO toDto(ExchangeRate s);
}
