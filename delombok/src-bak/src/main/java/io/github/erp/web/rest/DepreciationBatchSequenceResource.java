...
@RestController
@RequestMapping("/api")
public class DepreciationBatchSequenceResource {
    ...
    public ResponseEntity<DepreciationBatchSequenceDTO> updateDepreciationBatchSequence(
        ...
        return ResponseEntity
            .ok()
            .headers(
                HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, depreciationBatchSequenceDTO.getId().toString())
            )
            .body(result);
    }
